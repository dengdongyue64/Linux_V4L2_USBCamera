...
void getV4l2FrameParam(void* v4l2ctx){
	...
	int framerate[50],i=0;
	int ret=0;
	fmtdesc.index = 0;
	fmtdesc.type = type;
	fmtdesc.flags = V4L2_FMT_FLAG_EMULATED;
	while (ioctl(V4l2_Context->mCamFd, VIDIOC_ENUM_FMT, &fmtdesc) >= 0) {
		frmsize.pixel_format = fmtdesc.pixelformat;
		frmsize.index = 0;
		printf("*****************\t%d. %s  ******************\n",fmtdesc.index, fmtdesc.description);
		while (ioctl(V4l2_Context->mCamFd, VIDIOC_ENUM_FRAMESIZES, &frmsize) >= 0){
			if(frmsize.type == V4L2_FRMSIZE_TYPE_DISCRETE){
				...
				while ((ret = ioctl(V4l2_Context->mCamFd, VIDIOC_ENUM_FRAMEINTERVALS, &frmival)) == 0) {
					if (frmival.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
					    framerate[i++] = frmival.discrete.denominator/frmival.discrete.numerator;
					    printf("%dx%d : %d fps    %d/%d \n",frmival.width,frmival.height, 
									framerate[i-1],frmival.discrete.denominator,frmival.discrete.numerator);
...
int V4l2MapBuffer(void* v4l2ctx)
{
	...
	for (count = 0; count < V4l2_Context->mBufferCnt; count++)
	{
		...
		if (NULL == V4l2_Context->mMapMem.mem[count])
		{
